...
namespace MazeGame
{
    public class UButton : Button
    {
        ...
        public UButton(string text, string name, int tag, DockStyle dockStyle, EventHandler handler, int fontSize = 12, int degree = 0)
        {
            Height = 150;
            Text = text;
            Name = name;
            Font = new Font("Fira Code Retina", fontSize, FontStyle.Regular);
            Tag = tag;
            Dock = dockStyle;
            Handler += handler;
            this.degree = degree;
